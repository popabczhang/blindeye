...
#            i += 3
    ...
    return compound(tris, origin=vector(0,0,0), pos=vector(0,0,0), axis=vector(1,0,0))

if __name__ == '__main__':
    # RZ
    curve(pos=[vector(0,0,0), vector(1000,0,0)], color=color.red, radius=10)  # x
    curve(pos=[vector(0,0,0), vector(0,1000,0)], color=color.green, radius=10)  # y
    curve(pos=[vector(0,0,0), vector(0,0,1000)], color=color.blue, radius=10)  # z
    t = stl_to_triangles('t02.stl')
    # t.size *= 1
    # t.pos = vec(0,0,0)
    t.color = color.white
    t.rotate(angle=-0.5*pi, axis=vector(1,0,0))
    world_pos = t.compound_to_world(vector(0,0,1))
    print(f'world_pos: {world_pos}')
    compound_pos = t.world_to_compound(vector(0,0,1))
